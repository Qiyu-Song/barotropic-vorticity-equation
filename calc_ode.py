...
class ode:
    def __init__(self, ic=None, tendfunc=None, dt=0.01, steps=1000, scheme="", debug=0):
        ...
        try:
            print("start")
            print(np.shape(ic))
            if np.shape(ic) == np.shape(tendfunc(ic)):
                self.shape = np.shape(ic)
        except:
            print(np.shape(ic))
            print(np.shape(tendfunc(ic)))
        ...
        self.traj = np.expand_dims(np.zeros(np.shape(ic)), 0).repeat(self.steps, axis=0)
        #self.traj = np.ndarray(shape=(self.steps, self.shape), dtype=float)
